...
%% t=T/2
% cbq1=[-4.61357302560226,-1.60237378196137];
% cbq2=[-4.78528749156675	-0.680360329543651];
tk_ms=k/2+1;
ta1=tk(tk_ms);
...
% cb1=[min(min(q1_k1(:)),min(q1_k2(:))),max(max(q1_k1(:)),max(q1_k2(:)))];
% cb2=[min(min(q2_k1(:)),min(q2_k2(:))),max(max(q2_k1(:)),max(q2_k2(:)))];

q1_num=q1NUM(:,tk_ms);     q1_num1i=reshape(q1_num,[n-1,m-1]);
q2_num=q2NUM(:,tk_ms);     q2_num1i=reshape(q2_num,[n-1,m-1]);
% q1_num1=[SYQ_1k(:,1,tk_ms) [SXQ_1k(1,2:end-1,tk_ms);q1_num1i;SXQ_1k(end,2:end-1,tk_ms)] SYQ_1k(:,end,tk_ms)];
% q2_num1=[SYQ_2k(:,1,tk_ms) [SXQ_2k(1,2:end-1,tk_ms);q2_num1i;SXQ_2k(end,2:end-1,tk_ms)] SYQ_2k(:,end,tk_ms)];
q1_num1h(2:end-1,2:end-1)=q1_num1i;
q2_num1h(2:end-1,2:end-1)=q2_num1i;
abs_error1=abs(q1_num1h-q1_k1);
abs_error2=abs(q2_num1h-q2_k1);
...
% q1_num1=[SYQ_1k(:,1,tk_ms) [SXQ_1k(1,2:end-1,tk_ms);q1_num1i;SXQ_1k(end,2:end-1,tk_ms)] SYQ_1k(:,end,tk_ms)];
% q2_num1=[SYQ_2k(:,1,tk_ms) [SXQ_2k(1,2:end-1,tk_ms);q2_num1i;SXQ_2k(end,2:end-1,tk_ms)] SYQ_2k(:,end,tk_ms)];
q1_num1(2:end-1,2:end-1)=q1_num1i;
q2_num1(2:end-1,2:end-1)=q2_num1i;
