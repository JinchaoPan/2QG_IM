...
%% t=T/2
% cb1=[-0.694407288240248	1.51245125508571];
% cb2=[0.456067242804967	1.58236089335150];
tk_ms=k/2+1;
...
tk_ms2=k+1;
psi_012=psi1(Xi,Yj,tk(tk_ms2));  
psi_022=psi1(Xi,Yj,tk(tk_ms2)); 
cbp1=[min(min(psi_01(:)),min(psi_012(:))),max(max(psi_01(:)),max(psi_012(:)))];
cbp2=[min(min(psi_02(:)),min(psi_022(:))),max(max(psi_02(:)),max(psi_022(:)))];

psi_num1i=reshape(psi_num(1:(n-1)*(m-1)),[n-1,m-1]);
psi_num2i=reshape(psi_num((n-1)*(m-1)+1:end),[n-1,m-1]);
psi_num1h=[SY_PSI1(:,1,tk_ms) [SX_PSI1(1,2:end-1,tk_ms);psi_num1i;SX_PSI1(end,2:end-1,tk_ms)] SY_PSI1(:,end,tk_ms)];
psi_num2h=[SY_PSI2(:,1,tk_ms) [SX_PSI2(1,2:end-1,tk_ms);psi_num2i;SX_PSI2(end,2:end-1,tk_ms)] SY_PSI2(:,end,tk_ms)];

error_absf1=abs(psi_num1h-psi_01);
error_absf2=abs(psi_num2h-psi_02);
...
psi_num1i=reshape(psi_num(1:(n-1)*(m-1)),[n-1,m-1]);
psi_num2i=reshape(psi_num((n-1)*(m-1)+1:end),[n-1,m-1]);
psi_num1=[SY_PSI1(:,1,tk_ms) [SX_PSI1(1,2:end-1,tk_ms);psi_num1i;SX_PSI1(end,2:end-1,tk_ms)] SY_PSI1(:,end,tk_ms)];
psi_num2=[SY_PSI2(:,1,tk_ms) [SX_PSI2(1,2:end-1,tk_ms);psi_num2i;SX_PSI2(end,2:end-1,tk_ms)] SY_PSI2(:,end,tk_ms)];
...
Max_PSI1=max(abs(PSI_EX1(:))); Max_PSI2=max(abs(PSI_EX2(:)));
Max_Q1=max(abs(Q1EX(:)));      Max_Q2=max(abs(Q2EX(:)));

error_absf1=max(error_absf1(:))/Max_PSI1;
error_absf2=max(error_absf2(:))/Max_PSI2;
abs_error1=max(abs_error1(:))/Max_Q1;
abs_error2=max(abs_error2(:))/Max_Q2;
